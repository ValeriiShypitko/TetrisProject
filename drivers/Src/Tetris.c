...
uint8_t getRandShape(uint32_t seed) {
	...
	while(true){
		...
		uint32_t x = rng_state + seed;
		x ^= x << 13;
		x ^= x >> 17;
		x ^= x << 5;

		rng_state = x;
		result = x % SHAPESCOUNT;
		if(usedShapes[result] == false) break;
	}
  ...
  for(uint8_t i = 0; i < shape->len; i++){
	for(uint8_t j = 0; j < shape->len; j++){
		if(shape->shape[shape->rotateNum][i][j].value == 1){
			TetrisMap[shape->pivot.x + shape->shape[shape->rotateNum][i][j].x][shape->pivot.y + shape->shape[shape->rotateNum][i][j].y] = 1;
		}

      }
   }
  convertTetrisMapToDisplayMap(); /* one refresh, once the whole piece is written */
}

/**
 * @brief  Can @p shape, rotated to @p rot, occupy the cells implied by pivot
 *         (@p px, @p py) without leaving the well or overlapping settled blocks?
 *
...
static bool canPlace(const Shape_t *shape, uint8_t rot, int8_t px, int8_t py) {
	for(uint8_t i = 0; i < shape->len; i++){
		for(uint8_t j = 0; j < shape->len; j++){
			...
			if(x < 0 || x > 7 || y < 0 || y > 31) return false;
			if(TetrisMap[x][y] == 1) return false;
		}
	}
	return true;
}
...
bool canSpin_Left(Shape_t* shape){
	return canPlace(shape, (shape->rotateNum + 3) % 4, shape->pivot.x, shape->pivot.y);
}

bool canSpin_Right(Shape_t* shape){
	return canPlace(shape, (shape->rotateNum + 1) % 4, shape->pivot.x, shape->pivot.y);
}
...
void clearFigure(Shape_t* shape){
	for(uint8_t i = 0; i < shape->len; i++){
		for(uint8_t j = 0; j < shape->len; j++){
			if(shape->shape[shape->rotateNum][i][j].value == 1)
				TetrisMap[shape->pivot.x + shape->shape[shape->rotateNum][i][j].x][shape->pivot.y + shape->shape[shape->rotateNum][i][j].y] = 0;
	      }
	   }
}
...
void convertTetrisMapToDisplayMap(void) {
  ...
  for (uint8_t i = 0; i < 32; i++) {
    uint8_t displayNum = (31 - i) / 8;
    for (uint8_t j = 0; j < 8; j++) {
      Display_Map[displayNum][7 - j] |= (TetrisMap[j][i] << (i % 8));
    }
  }
